...
const ClientComponent = ({
  ...
  const [voopanList, setVoopanList] = useState<any[]>([]);
  ...
  useEffect(() => {
    const getCurrentLocation = async () => {
      try {
        ...
        if (categoryList?.length > 0 && voopanList?.length > 0 && position) {
    ...
    try {
      ...
      if (Array.isArray(resultEvent?.data)) {
        const templist = resultEvent.data.map((item) => ({
          ...item,
          category_id: item?.promoter_voopon?.event_data?.category_id,
          subcategory_id: item?.promoter_voopon?.event_data?.subcategory_id,
        }));

        setVoopanList(filterEvent(templist, categoryMainList));
  ...
  useEffect(() => {
    ...
    if (appliedFilter.isSearchApply) {
      let newVoopons = tempList.filter((evt): any =>
        evt?.promoter_voopon?.voopons_name
          .toLowerCase()
    ...
    if (locationFilter.length > 0) {
      let newVoopons = tempList.filter((obj) => {
        const locationParts = obj?.promoter_voopon?.location?.split(", ") || [];
      ...
      <div className="user-dashboard-data">
        <div className="user-my-favorites">
          <div className="deals-inner-search">
            <form onSubmit={handleVoopanSearch}>
        ...
        <div className="user-my-voop-eve">
          <div className="row">
            <div className="col-lg-12 mb-3" ref={targetDivRef}>
                ...
                return (
                  <div key={item.id} className="col-lg-4">
                    <div className="voopan-box">
                      {checkExpirationStatus(
                        item?.promoter_voopon?.voopons_valid_thru
                      ...
                      <div className="voopon-logo">
                        <Image
                          ...
                          height={125}
                          src={
                            item?.vooponimageone?.image_name
                              ? `${BASE_URL}${item?.vooponimageone?.image_name}`
                      ...
                      <div className="voopon-heading">
                        {" "}
                        {item?.promoter_voopon?.voopons_name}
                      </div>
                      <h5>{item?.promoter_voopon?.voopons_description}</h5>
                      <p>
                        Valid Thru:{" "}
                        {DateTime.fromFormat(
                          item?.promoter_voopon?.voopons_valid_thru,
                      ...
                      <a
                        ...
                        role="button"
                        href={`/my/voopons/${item?.promoter_voopon_id}?promoter_id=${item.promoter_id}`}
                      >
                        View More
                      </a>
                    </div>
